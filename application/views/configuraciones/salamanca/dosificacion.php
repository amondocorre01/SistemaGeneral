<div id="tab-<?=$id?>"> 
      ...
      <div id="opciones-salamanca">
          ...
          <ul>
              <li><a href="#opcion-1-salamanca<?=$id?>"> Dosificacion </a></li>
              <li><a href="#opcion-2-salamanca<?=$id?>"> Impresion </a></li>
              <li><a href="#opcion-3-salamanca<?=$id?>"> Venta Programada </a></li>
          </ul>
    
          <div id="opcion-1-salamanca<?=$id?>"> 

            <div class="row">
                <div class="col-4">
                    <?=form_button(['onclick'=>'new_salamanca()', 'content'=>'<i class="las la-plus-circle la-1x palette-White text"></i>'.'<span class="palette-White text">Nuevo</span>', 'class'=>'btn btn-md btn-danger']);?>
                </div>
            </div>

            <div class="row">
              <div class="col-12">
              <table id="table-salamanca" class="table table-bordered table-striped dataTable dtr-inline responsive">
          ...
          <div id="opcion-3-salamanca<?=$id?>"> 
        
          </div>
      </div>
</div>

<div class="modal fade" id="nuevo_salamanca" tabindex="-1" role="dialog" aria-labelledby="nombre3" aria-hidden="true">
  <div class="modal-dialog modal-xs" role="document">
    <div class="modal-content">
      ...
      <?php $data['id'] = $id?>
      <?php echo $this->load->view('configuraciones/salamanca/nueva', $data, TRUE);?>
           
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
        <button type="button" class="btn btn-danger" onclick="confirmar_register_salamanca(<?=$id?>)">Confirmar</button>
      </div>
    </div>
  </div>
</div>
      ...
      <script>
        function new_salamanca(){
            $('#nuevo_salamanca').modal('show');
        }

            function confirmar_register_salamanca(id) {
            ...
            url = '<?=site_url('nueva-salamanca')?>'

            $.post(url, datos)
            .done(function(data) {
            $('#nuevo_salamanca').modal('hide');
            table_salamanca.ajax.reload();
            });
            }
        ...
        var table_salamanca = $('#table-salamanca').DataTable({
        responsive: true,
        ajax: { url: '<?=site_url('get-salamanca')?>', dataSrc:"" },
