<div id="tab-<?=$id?>"> 
      ...
      <div id="opciones-megacenter">

          <ul>
              <li><a href="#opcion-2-megacenter<?=$id?>"> Impresion </a></li>
              <li><a href="#opcion-4-megacenter<?=$id?>"> Mensajes </a></li>
          </ul>
    
         
          <div id="opcion-2-megacenter<?=$id?>">
             <?php echo $this->load->view('configuraciones/megacenter/impresion', null, TRUE);?> 
          </div>
         

			 <div id="opcion-4-megacenter<?=$id?>"> 
				...
				<div class="row">
					<div class="col-3">
						<?=form_button(['onclick'=>'message_megacenter('.$id.')', 'content'=>'<i class="las la-check la-1x palette-White text"></i>'.'<span class="palette-White text">Guardar</span>', 'class'=>'btn btn-md btn-danger']);?>
					</div>
				</div>
          </div>
      </div>
      ...
      <script>
        ...
        $('#opciones-megacenter').responsiveTabs({
            startCollapsed: 'accordion'
        });

        var show_update = 0;

	 function message_megacenter(id) {
		...
		$.post("<?=site_url('set-message-megacenter')?>", {id:id, factura:factura, recibo:recibo, comanda:comanda })
			.done(function( data ) {
