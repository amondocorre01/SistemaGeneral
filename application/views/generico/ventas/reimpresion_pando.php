        ...
        <div class="row">
            <div class="col-offset-3 col-md-3">
                <form action="<?=current_url()?>" method="GET" id="form_fecha">
                    <input name="fecha" type="date" class="form-control" id="fecha" onchange="send()">
...
{ title:"Accion", className: 'check-boy', data: null, orderable: false, className: 'text-center',
    render: function (data, type, full, meta) 
    {
       ...
       if(data.ANULADO == 0){
        ...
        if(io == true) {
            button += '<button class="btn btn-primary btn-info btn-md" onclick="reimprimirFacturaCarta('+data.NUMERO_FACTURADO+')" title="Imprimir factura carta">';
...
function guardarAnularFactura(element){
    ...
    function reimprimirFacturaCarta(id) {
        var url= "<?=site_url('reimprimir-factura')?>"+"/"+id;
        window.open(url,'_blank');
    } 
